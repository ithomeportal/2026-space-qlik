"""Code-made report: Admin - Access Log Doors.

Department-locked sibling of `HR - Access Log Doors` for the Admin audience
(2026-05-07, mirrors the existing DFW lock). Business logic is identical
(first punch / day, expected-arrival rule, integer minute delta) but every
endpoint hard-codes `dep = 'Admin'` so:

  * users with the `AdminFinance` TagRole can only ever see the Admin
    department, never widen the scope by tampering with query strings
  * the front-end drops the Department dropdown entirely
  * the by-job-title chart replaces the by-department bars (single dept
    would be useless)
  * the rolling-30-day trend stays locked to Admin — unlike the HR
    original which is deliberately global
...
_ADMIN_DEPT_GATE = "AND dep = 'Admin'"


router = APIRouter(tags=["admin-access-doors"], prefix="/custom/admin-access-doors")
...
async def filters(
    request: Request,
    _user: dict = Depends(require_report_access("admin-access-doors")),
):
    """Distinct job titles + employee names seen in Admin over the
    last 90 days. Department dropdown intentionally omitted — the report is
    locked to Admin server-side."""
...
async def kpis(
    ...
    params: list = [s, e]
    filters_sql = _build_admin_filters_sql(params, name, job_title)
    ...
    row = await pool.fetchrow(
        ...
        SELECT
          ...
          COUNT(*)                                                     AS total_rows
        FROM scored
        WHERE 1=1 {_ADMIN_DEPT_GATE} {filters_sql}
        """,
        *params,
    )
...
# --------------------------------------------------------------------------
# /trend-30d  -- rolling 30-day line, locked to Admin
# --------------------------------------------------------------------------
...
async def trend_30d(
    ...
    Admin. Ignores the user-selected date filter (rolling 30d) but
...
async def by_job_title(
    ...
    """On-Time vs Out-of-Time per job title within Admin for the
    ...
    filters_sql = _build_admin_filters_sql(params, name, None)
