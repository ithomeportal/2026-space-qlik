"""Code-made report: XRay DFW Mng — management X-Ray for DFW sub-teams.

Sibling to ``xray_corp.py`` (CORP version). Same 6 tabs, same KPI math,
filtered to the DFW division.

Scope (vs CORP version):
- ``v4.team_id = 'TEAM-DFW'`` (single division — CORP uses TEAM1..5)
- Sub-team filter on ``v4.team`` ∈ {TM1, TM2, TM3, TM4} (CORP uses team_id pill)
- Scorecard sub-team via ``scorecard.team_dfw`` ∈ {TM1..TM4}
- Excludes UNILINK + OILTEX (CORP excludes only OILTEX)
- Profit-TM is computed directly from v4 (``daily_production_budget_report``
  is CORP-only, see CLAUDE.md / SPEC-CUSTOM-REPORTS §v4 sparseness rule)
- Savings trio maps customers via TEAM-DFW (CORP maps via TEAM1..5)
- TU% goal denominator uses 4 sub-teams (vs 5 CORP teams) when ``All`` selected;
  same per-team goals (25 day / 125 week / 500 month) — kept identical so
  Bruno's targets stay comparable.

Frontend ``xray-dfw-api.ts`` calls these endpoints with the same query
contract as CORP, except it sends ``sub_teams=TM1,TM2`` (CSV) instead of
``team=TEAM2`` — so the URL surface is intentionally different.
"""
...
XRAY_ROLES = ("CEO", "Executive", "DFW", "Operations", "Finance")
...
# Throughput Utilization goals — per sub-team.
TU_DAY = 25
TU_WEEK = 125
TU_MONTH = 500
...
def _parse_csv(raw: Optional[str], allowed: tuple[str, ...]) -> list[str]:
    """Parse a CSV of sub-team values, dropping anything outside the allowed list."""
    if not raw:
        return []
...
def _scope_where(
    ...
    ``sub_teams`` is the optional pill selection — empty list means "all
    DFW sub-teams" (no extra predicate). ``v4.team`` is varchar; we use
...
def _scorecard_cte(kind: str, sub_teams: list[str]) -> str:
    """Return a CTE that rolls scorecard up into per-order OTP / OTD counts.
    ...
    teams_sql = _lit(DFW_TEAMS, width=8)
    companies_sql = _lit(DFW_COMPANIES, width=4)
    statuses_sql = _lit(OPEN_STATUSES, width=1)

    sub_filter = ""
    if sub_teams:
        # team_dfw is varchar(512) — TRIM equality keeps it sargable enough
        # given the small cardinality (4 values) of the DFW filter set.
        in_list = ",".join(f"'{s}'" for s in sub_teams)
        sub_filter = f"\n      AND TRIM(team_dfw) IN ({in_list})"
    ...
    WHERE team_id    IN ({teams_sql})
      ...
      AND edi_standard_code IN ({codes_sql}){sub_filter}
...
async def kpis(
    ...
    # ---- Profit-TM (current calendar month) — direct v4 sum -------------
    # daily_production_budget_report is CORP-only, so DFW reads margin_amt
    # straight from v4. v4 lags in-progress current-month loads but is the
    ...
    sub_filter_sql = ""
    if sub_team_list:
        sav_params.append(sub_team_list)
        sub_filter_sql = f" AND TRIM(team) = ANY(${len(sav_params)})"

    sav_task = pool.fetchrow(
        f"""
        WITH customer_team AS (
            SELECT customer_name, team FROM (
                SELECT
                    TRIM(customer_name) AS customer_name,
                    TRIM(team)          AS team,
                    ROW_NUMBER() OVER (
                        PARTITION BY TRIM(customer_name)
                        ORDER BY COUNT(*) DESC, TRIM(team)
                    ) AS rn
                FROM public.mcleod_gld_budget_report_v4
                WHERE TRIM(team_id) = 'TEAM-DFW'{sub_filter_sql}
                GROUP BY TRIM(customer_name), TRIM(team)
            ) ranked
            WHERE rn = 1
        )
        SELECT
    ...
    async def _window(d_from: date, d_to: date, capacity: int) -> dict:
        ...
        rows = await pool.fetch(
            ...
            WITH otp AS ({_scorecard_cte("otp", sub_team_list)}),
                 otd AS ({_scorecard_cte("otd", sub_team_list)}),
                 prod AS (
                    SELECT TRIM(br4.team) AS team_label,
            ...
            FROM prod
            GROUP BY ROLLUP(team_label)
            ORDER BY team_label NULLS FIRST
            """,
            *params,
        )
        totals = None
        ...
        for r in rows:
            ...
            label = r["team_label"]
            # Drop rows that don't fall under the DFW sub-team set (stale
            # legacy team labels or NULLs become "Totals" via ROLLUP NULL).
            if label is not None and label not in DFW_SUB_TEAMS:
                continue
            row_out = {
                "team": label or "Totals",
                "vol": vol,
                "rev": rev,
                "prof": prof,
                ...
                "tu": (vol / (capacity * (total_team_count if label is None else 1))) * 100.0,
...
# ---------------------------------------------------------------------------
# Tab 3 — Teams Breakdown (TM / TW / LW / L2W..L5W) by sub-team
# ---------------------------------------------------------------------------
...
async def teams_breakdown(
    ...
    teams_out: dict[str, dict[str, dict]] = {
        t: {r[0]: empty_bucket() for r in ranges} for t in DFW_SUB_TEAMS
    }
    ...
    return {
        "success": True,
        "data": {
            "teams": [{"team": t, **compute(teams_out[t])} for t in DFW_SUB_TEAMS],
