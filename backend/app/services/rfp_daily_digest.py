...
def _delta_pill(arrow: str, text: str, tone: str, prev_label: str, prev_value: str) -> str:
    ...
    return f"""
    <tr>
      <td style="padding:6px 0;font-size:12px;color:#374151;">
        <span style="display:inline-block;background:{bg};color:{fg};padding:2px 8px;border-radius:999px;font-size:11px;font-weight:700;letter-spacing:0.02em;">{arrow} {text}</span>
        <span style="color:#6B7280;font-size:11px;margin-left:8px;">vs {prev_label} · {prev_value}</span>
      </td>
    </tr>
    """
...
def _kpi_card_money(label: str, current: float, comparisons: list[tuple[str, float]]) -> str:
    ...
    return f"""
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FFFFFF;border:1px solid #E5E7EB;border-radius:8px;margin-bottom:16px;">
      <tr><td style="padding:18px 20px;">
        <div style="font-size:10px;color:#6B7280;letter-spacing:0.1em;text-transform:uppercase;font-weight:700;">{label}</div>
        <div style="font-size:30px;font-weight:800;color:#111827;line-height:1.1;margin-top:6px;letter-spacing:-0.01em;">{_fmt_money(current)}</div>
        <div style="font-size:11px;color:#6B7280;margin-top:2px;">YTD ({date.today().year})</div>
        <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-top:14px;border-top:1px solid #F3F4F6;">
          {''.join(rows)}
        </table>
      </td></tr>
    </table>
...
def _kpi_card_ratio(label: str, current: float | None, comparisons: list[tuple[str, float | None]]) -> str:
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FFFFFF;border:1px solid #E5E7EB;border-radius:8px;margin-bottom:16px;">
      <tr><td style="padding:18px 20px;">
        <div style="font-size:10px;color:#6B7280;letter-spacing:0.1em;text-transform:uppercase;font-weight:700;">{label}</div>
        <div style="font-size:30px;font-weight:800;color:#111827;line-height:1.1;margin-top:6px;letter-spacing:-0.01em;">{_fmt_pct(current, decimals=2)}</div>
        <div style="font-size:11px;color:#6B7280;margin-top:2px;">YTD ({date.today().year})</div>
        <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-top:14px;border-top:1px solid #F3F4F6;">
          {''.join(rows)}
        </table>
...
def _convertio_table_html(table: dict[str, Any]) -> str:
    ...
    for r in table["rows"]:
        ...
        <tr style="border-top:1px solid #F3F4F6;">
          <td style="padding:7px 10px;font-size:12px;font-weight:600;color:#111827;font-variant-numeric:tabular-nums;">{r['ratio'] * 100:.2f}%</td>
          <td style="padding:7px 10px;text-align:right;font-size:12px;color:#374151;font-variant-numeric:tabular-nums;">{_fmt_money(r['revenue_awarded_at_ratio'])}</td>
          <td style="padding:7px 10px;text-align:right;font-size:12px;font-weight:700;background:{bg};color:{fg};font-variant-numeric:tabular-nums;">{_fmt_money(delta)}</td>
          <td style="padding:7px 10px;text-align:right;font-size:12px;color:#374151;font-variant-numeric:tabular-nums;">{_fmt_money(r['pot_rev_gp_15'])}</td>
        </tr>
        """)
    return f"""
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FFFFFF;border:1px solid #E5E7EB;border-radius:8px;">
      <tr><td style="padding:16px 18px 8px 18px;">
        <div style="font-size:13px;font-weight:700;color:#111827;">Convertio Ratio by Status Won and Lost</div>
        <div style="font-size:11px;color:#6B7280;margin-top:4px;">
          Potential Revenue Closed: <strong style="color:#111827;">{_fmt_money(table['pot_rev_closed'])}</strong>
          &nbsp;·&nbsp;
          Revenue Awarded: <strong style="color:#111827;">{_fmt_money(table['revenue_awarded'])}</strong>
        </div>
      </td></tr>
        ...
        <table cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
          <thead>
            <tr style="background:#ECFDF5;">
              <th style="padding:8px 10px;text-align:left;font-size:10px;color:#065F46;text-transform:uppercase;letter-spacing:0.05em;font-weight:700;">Convertio Ratio</th>
              <th style="padding:8px 10px;text-align:right;font-size:10px;color:#065F46;text-transform:uppercase;letter-spacing:0.05em;font-weight:700;">Revenue Awarded</th>
              <th style="padding:8px 10px;text-align:right;font-size:10px;color:#065F46;text-transform:uppercase;letter-spacing:0.05em;font-weight:700;">Actual vs Conv. Ratio</th>
              <th style="padding:8px 10px;text-align:right;font-size:10px;color:#065F46;text-transform:uppercase;letter-spacing:0.05em;font-weight:700;">Pot Rev GP 15%</th>
            </tr>
          </thead>
          <tbody>{''.join(rows_html)}</tbody>
        </table>
      </td></tr>
      <tr><td style="padding:0 18px 14px 18px;font-size:10px;color:#9CA3AF;">
    ...
    *,
    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:8px;margin-bottom:16px;">
      <tr><td style="padding:12px 16px;">
        <div style="font-size:10px;color:#6B7280;letter-spacing:0.08em;text-transform:uppercase;font-weight:700;">Period detail</div>
        <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-top:6px;font-size:11px;color:#374151;">
          <tr>
            <td style="padding:3px 0;"><strong style="color:#111827;">MTD</strong> · {_fmt_money(kpi_mtd['awarded_revenue'])} awarded · {_fmt_pct(kpi_mtd['awarded_convertio_ratio'])}</td>
          </tr>
          <tr>
            <td style="padding:3px 0;"><strong style="color:#111827;">QTD</strong> · {_fmt_money(kpi_qtd['awarded_revenue'])} awarded · {_fmt_pct(kpi_qtd['awarded_convertio_ratio'])}</td>
          </tr>
        </table>
      </td></tr>
    </table>
    ...
    <html><body style="margin:0;padding:0;background:#F3F4F6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
      <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#F3F4F6;">
        <tr><td align="center" style="padding:24px 12px;">
          <table cellpadding="0" cellspacing="0" border="0" width="760" style="max-width:760px;background:#FFFFFF;border-radius:10px;overflow:hidden;border:1px solid #E5E7EB;">
            <!-- Header -->
            <tr><td style="background:#1B3A5C;color:#FFFFFF;padding:22px 26px;">
              <div style="font-size:11px;letter-spacing:0.12em;text-transform:uppercase;opacity:0.75;">UNILINK Space · Daily Digest</div>
              <h1 style="margin:6px 0 4px 0;font-size:22px;font-weight:700;letter-spacing:-0.01em;">Performance for RFPs</h1>
              <div style="font-size:13px;opacity:0.85;">{today.strftime('%A, %B %d, %Y')} · 5:30 PM CST</div>
            </td></tr>
            ...
            <!-- Footer -->
            <tr><td style="background:#FFFFFF;padding:16px 26px 22px 26px;border-top:1px solid #E5E7EB;text-align:center;">
              <a href="{portal_url}/reports/rfp-performance" style="display:inline-block;background:#1B3A5C;color:#FFFFFF;padding:10px 18px;border-radius:6px;text-decoration:none;font-size:13px;font-weight:600;">Open the full report →</a>
              <div style="font-size:11px;color:#6B7280;margin-top:14px;">
                Source: <code style="font-family:'SF Mono',Menlo,Consolas,monospace;color:#111827;">automations_db.rfp_results_history</code>
