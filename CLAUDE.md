...
- Current catalog:
  - **eSavings from Carriers** — `/reports/esavings-carriers` · roles: CEO, Executive, Procurement, Finance, CORP, DFW · source `aivn_datalake_gold.carriers_savings_results_report` (n8n `PdZIaBQPGSLD4VWB`) · filters: Month + Division (CORP/DFW) + Corp Team (TEAM1..TEAM5) resolved via McLeod join (skipped when no filter — unfiltered totals unchanged) · Total Savings card shows $55k/division monthly goal progress · Team Summary table + dual-axis 9-month trend chart · see `docs/SPEC-CUSTOM-REPORTS.md`
