...
- Current catalog:
  - **eSavings from Carriers** — `/reports/esavings-carriers` · roles: CEO, Executive, Procurement, Finance, CORP, DFW · source `aivn_datalake_gold.carriers_savings_results_report` · populated by n8n `PdZIaBQPGSLD4VWB` · supports Month + Division (CORP/DFW) + Corp Team (TEAM1..TEAM5) filters; team resolved via same `mcleod_gld_budget_report_v4` dominant-team join used by budget-followup (join skipped when no filter, so unfiltered totals are unchanged)
