...
| 2026 Official Budget Follow Up | `/reports/budget-followup-2026` | CEO, Executive, Operations, Finance, CORP, DFW | `daily_production_budget_report` + v4 team map. **Bruno feedback 2026-04-28**: Range presets `Full 2026 / YTD / MTD / Custom` (MTD = 1st-of-month → today; default still month-to-yesterday). Monthly chart shows actual $ above every bar; new Last-12-Weeks chart underneath (ISO Mon-Sun, current excluded). 5 leaderboard cards between By-Team and By-Customer (Top/Worst Volume + Top/Worst Profit `where budget>0` + Non-Budget Active `where profit_budget=0 AND profit_actual>0`, all respecting filters). By-Customer is tabbed: Overview / Revenue / Profit (no Loads tab). Overview splits Loads A∣B (integers), Margin A∣B, +Profit B, click-to-sort. Revenue/Profit trajectory tabs use **Mon-Fri working days, no holidays** (`_count_workdays` in `budget_followup.py`) for AVG×LastMonth divisor + Projected days-remaining; numerator sums stay calendar-day. AVG×Week=/3, AVG×Day=/14 are spec'd literally. Avg+Projected always anchor to *now*, ignore the Range filter. |
...
15. **Code-Made Reports** — Non-Qlik reports via `report_type='custom'`; current: eSavings from Carriers, 2026 Official Budget Follow Up, XRay CORP Mng, CEO Executive, HR Access Doors, DFW Access Doors, Podium Set DFW, Top Losses Lanes, Attrition WoW, OPs Margins, OPs Direct Compare, Sales- Attrition to OPs, OPs Customer Score, VoIP Calls Logs, Track Award Loads, Performance for RFPs, Risk Asss for Carriers, IT Tickets Mgmt
