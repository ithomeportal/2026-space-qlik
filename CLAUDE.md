...
- **Scorecard uses portal-owned mirrors, NOT the shared Spark source** (source bloats + collapses PU/DEL to one incident). Two n8n-refreshed tables: `mcleod_gld_scorecard_portal` (`Ou7GmT47H8IuNoGj`, TRUNCATE+load, anti-bloat) and `mcleod_gld_scorecard_incidents_portal` (incident-grain, recovers PU+DEL fails the movement-grain PK hid). All scorecard reports (ops-customer-score, xray_corp/dfw, ops-portal-overview, Bonus Calculator via `xray_corp._scorecard_cte`) read the mirrors. ⚠ **On a new incident-table deploy: import+activate the n8n workflow FIRST or the report 500s.** `daily_scorecard_mirror_check` (6:15 AM CST) alerts on stale refresh. Full detail + root-cause: SPEC-CODE-RULES §43, SPEC-RELIABILITY §3d, `docs/BRUNO-scorecard-rootcause-2026-06-15.md`.
