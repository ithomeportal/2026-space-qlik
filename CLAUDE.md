...
- **`mcleod_gld_scorecard` heap-bloat** (2026-06-02): the McLeod→gold loader DELETEs+reinserts → table bloated 71× (3,488MB heap for ~62MB live, 0.45 tuples/page) → `_scorecard_cte` read ~430MB twice/request. One-off `VACUUM (FULL, ANALYZE)` ran as **avnadmin off-hours 2026-06-02** (→62MB, 17.4 tuples/page; needs master role + ACCESS EXCLUSIVE lock — local only per SPEC-CODE-RULES §8). It WILL creep back (loader unchanged), so the backend (SELECT-only on gold, can't `VACUUM FULL`) runs a **weekly watchdog `weekly_scorecard_bloat_check` (`_scheduled_scorecard_bloat_check`, Mon 6:15 AM CST)** that emails Diego when size crosses 500MB to trigger the next manual repack. Permanent fix = switch the loader to TRUNCATE+reload. Hand-off SQL: `docs/ops-portal-overview-indexes-avnadmin.sql`.
...
- `daily_losses_alert` 07:00 CST (Resend); `daily_rfp_digest` 17:30 CST Mon-Fri (MS Graph, `admin-ms-api` app); `weekly_scorecard_bloat_check` Mon 06:15 CST (Resend → Diego, datalake heap-bloat watchdog). Reuse `FONT_STACK`/`MONO_STACK` for HTML email (Outlook reset)
