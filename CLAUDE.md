...
- Key SQL/code rules (SPEC-CODE-RULES §): no-TRIM sargability §1 · CST clock pin §2 · v4 sparseness §3 · date-decode clamp + NaN/Inf guards + bind `datetime`/`date` (never `str`) to `::timestamp`/`::date` params §4 · LATERAL first-match §5 · KPI=detail §16 · ratio-pivot numerator+denominator §33 · atomic wire-field rename §34 · per-user `user_id` scoping §35 · per-tab chart series §36 · grain toggle §37 · v4 profit = SUM(margin_amt), no total_charge≠0 filter (accessorials) §39 · direct-call endpoint shims must forward EVERY param (FastAPI `Query()` default isn't applied on a Python call → 500) §40 · Bonus Calculator display bracket % may intentionally ≠ payout money (gated sub-100-load weeks show the % but pay $0) — check LATEST bonus-spec changelog before "reconciling" §41 · `mcleod_gld_customer_windows` uses `orig_`/`dest_` prefix (NOT v4's `origin_`), join on `TRIM(UPPER(id))`, sentinel-guard >2000 §42 (⚠ the "pre-aggregate not LATERAL" half of §42 was REVERSED 2026-06-02 — functional index `idx_customer_windows_id_upper` now exists, so /by-order uses a correlated LATERAL) · never `::date`-cast an indexed timestamp in a WHERE bound (kills the index → full scan; use `>= $s AND < ($e::date + INTERVAL '1 day')`) + every pool gets `command_timeout` < proxy abort + heavy multi-panel pages need `keepPreviousData` + never retry a client abort §43 · pinned Totals rows read a server-side FULL-universe aggregate (`data.totals`), never a client `reduce()` over LIMIT-capped rows; sibling tables that must reconcile share one totals query §44
...
- **⚠ `/api/health` warms only the DYNO, not the DB.** It runs zero queries, so the `aivn_datalake_gold` page cache stays cold → first heavy report hit paid 20-28s (2026-06-02 slowness incident). The `datalake_warmup` APScheduler job (`_scheduled_datalake_warmup`, **every 5 min**) runs the 3 heaviest ops-portal-overview queries in-process (httpx `ASGITransport`) to keep shared buffers hot. Compensator, not a cure — if the McLeod→gold loader bulk-loads continuously on the shared Aiven instance it'll out-evict the warmup. See SPEC-CODE-RULES §43 + SPEC-RELIABILITY §3c.
- **`mcleod_gld_scorecard` bloat → portal-owned mirror** (2026-06-02): the shared Spark source UPDATE-churns (71M updates/123k rows, fillfactor=100, 92% HOT but no page headroom) → 71× heap bloat (3,488MB; `_scorecard_cte` read ~430MB twice/request). NOT delete+reinsert (`n_tup_del=0`). We don't own the Spark pipeline and other consumers may use the table, so rather than ALTER it the portal now reads a **portal-owned TRUNCATE+load mirror `mcleod_gld_scorecard_portal`** (gold, same schema, `GRANT SELECT` to sa_dfrodriguez), refreshed by **n8n workflow `Ou7GmT47H8IuNoGj`** ("Refresh mcleod_gld_scorecard_portal (poll-on-change)", every 15 min, reloads only when source `pg_stat` advances). The mirror can't bloat and the heavy source scan happens once/refresh off the user path. All scorecard reports read the mirror (18 `FROM` sites repointed). Health watchdog flipped from bloat→freshness: **`daily_scorecard_mirror_check` (6:15 AM CST)** emails Diego if the n8n refresh stalls. A one-off `VACUUM FULL` on the source ran 2026-06-02 (now moot for the portal). See SPEC-CODE-RULES §43 + SPEC-RELIABILITY §3d.
- **`mcleod_gld_scorecard` movement-grain collapse → incident-grain table** (2026-06-15, Bruno PDF): the scorecard PK is `(id, company_id, movement_id)` with **no `stop_type`**, so the Spark upsert keeps only ONE incident per movement — a load with both a Pickup and a Delivery service-fail loses one and which survives flips each load ("doesn't show in pickup or Delivery after refresh"; closed months drift). Fix: **OPs Customer Score now reads `mcleod_gld_scorecard_incidents_portal`** — a new portal-owned table built by n8n (clone of `Ou7GmT47H8IuNoGj`, JSON at `BOT/n8n-mcp/workflow_backups/NEW_scorecard_incidents_portal_mirror_20260615.json`): scoped scorecard order rows (denominator + dates + carrier) LEFT JOIN on full key to `mcleod_gld_incidents_v3` (one row per real incident). Denominator unchanged (97,649 orders both ways); recovers ~850 PU + ~653 DEL YTD fails the collapse was hiding (on-time % corrects slightly down). `''` stop-type bucket dropped (incidents carry real stop types). ⚠ **Deploy order: import+activate the n8n workflow FIRST**, else the report 500s (table missing). **Sibling reports migrated same day (2026-06-15):** `xray_corp`/`xray_dfw`(+`xray_dfw_team`)/`ops_portal_overview` `_scorecard_cte` repointed to the incident table + `''` bucket dropped → OTP/OTD now equals ops-customer-score (v4 denominator unchanged — it already coincides). **Bonus Calculator** inherits via `xray_corp._scorecard_cte`: `monthlyServicePct` dropped slightly (correction; May TEAM3 96.22%→95.79%, T&T bracket 0.8→0.7) — shipped with explicit OK, §41 payout sensitivity. Closed-month source churn (Bruno #3) is a separate snapshot task. Spark source itself is NOT ours to fix — root-cause note for Bruno at `docs/BRUNO-scorecard-rootcause-2026-06-15.md`.
...
10. **Code-Made Reports** — All reports are `report_type='custom'` Next.js routes. Current catalog: eSavings from Carriers, 2026 Official Budget Follow Up, XRay CORP Mng, XRay DFW Mng, XRay DFW TM1..TM4, CEO Executive, HR Access Doors, DFW Access Doors, Admin Access Doors, Podium Set DFW, DFW Podium Top, DFW Podium Top TM1..TM4, Top Losses Lanes, Attrition WoW, OPs Margins, OPs Direct Compare, Sales- Attrition to OPs, OPs Customer Score, VoIP Calls Logs, Track Award Loads, Performance for RFPs, Risk Asss for Carriers, IT Tickets Mgmt, Admin Aging Cashflow, Ops Portal - Overview, KAM Performance - DFW, Bonus Calculator, Reports Index, CEO Cockpit, Carrier SMS Score. **Full per-report spec in `docs/SPEC-CUSTOM-REPORTS.md`.**
