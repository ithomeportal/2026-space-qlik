...
- **Seed role grants apply ONLY on first report creation** (`xmax = 0` check in `seed_custom_reports`). The admin UI (`/admin/reports`) is the sole authority for TagRole access on existing reports — editing `CUSTOM_REPORTS.roles` only affects brand-new reports; granting on an existing report is a direct `role_report_access` write. See SPEC-CODE-RULES §15
...
- Render free tier cold-starts 30–60s (keep-warm above); proxy retries GET 5xx 3×, React Query 5×, skip 401/403; favicon backfill is a background task (never blocks lifespan)
