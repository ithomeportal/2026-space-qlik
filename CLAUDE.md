...
- **v4 sparseness rule** — `mcleod_gld_budget_report_v4` historically lagged for the current month; for executive roll-ups (KPIs, Summary by Team, All Teams Performance, Profit-TM) read `daily_production_budget_report` (6h n8n refresh) joined to a customer→team mapping derived from v4. Keep v4 for load/lane-level panels only. **Caveat (2026-04-27)**: `daily_production_budget_report` is **CORP-only** (Team IDs `TEAM1..TEAM5`, zero DFW customers). Any Overview-style query that needs DFW must `UNION ALL` v4-DFW production into the source — see `_production_cte()` in `ceo_executive.py`. CORP parity (v4 vs budget_report) is 0% Jan-Mar 2026 and 0.17% in the current month, so the union is safe with no double-counting (empirically zero customer overlap).
- **Sargability for `origin_actual_departure`** — never write `origin_actual_departure::date BETWEEN $s AND $e` in WHERE/JOIN — the cast on the column kills the btree index `idx_v4_dep` and forces a full seq-scan of v4 (380 MB). Use the half-open form `origin_actual_departure >= $s AND origin_actual_departure < ($e::date + 1)`. Same parity, ~30× faster on every panel that filters by date.
- **First-match-per-key joins** — for "first/min/max row per (key1,key2)" patterns (e.g. first movement per `(order_id, company_id)` to get carrier name), use `LEFT JOIN LATERAL ( ... ORDER BY ... LIMIT 1 ) ON TRUE`, not `ROW_NUMBER() OVER (PARTITION BY ...) WHERE rn=1` in a CTE. The CTE form pre-aggregates the entire secondary table with a window function (5.9s for /orders), the LATERAL form does one index seek per outer row (~150ms). Needs a supporting btree on `(key1, key2, sort_col)` — for movement that's `idx_movement_order_company_mv (order_id, company_id, movement_id)`.
