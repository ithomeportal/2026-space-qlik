...
import type { ScopedAccessJobTitleBar } from "@/lib/scoped-access-api"

interface Props {
  rows: ScopedAccessJobTitleBar[]
...
// Same visual as the HR DeptBarChart, but bars are per job-title since these
// reports are locked to a single department (or a fixed set of job titles).
export function AccessJobTitleBarChart({ rows, scopeLabel }: Props) {
  if (rows.length === 0) {
    return (
    ...
    <div className="rounded-lg border border-[#E5E7EB] bg-white p-3">
      <div className="mb-2 flex items-center gap-4 text-[10px] text-[#374151]">
        ...
        <span className="ml-auto text-[#9CA3AF]">
          {scopeLabel} · ignores job-title filter
        </span>
      </div>
