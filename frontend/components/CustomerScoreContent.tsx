...
import { fmtCount, fmtTimestamp } from "@/app/reports/ops-customer-score/format"
import { PuOverview } from "@/app/reports/ops-customer-score/tabs/PuOverview"
import { DelOverview } from "@/app/reports/ops-customer-score/tabs/DelOverview"
import { Detail } from "@/app/reports/ops-customer-score/tabs/Detail"
...
interface Props {
  /** Backend router prefix, e.g. "custom/ops-customer-score-t1". Defaults to
   *  the cross-team router via the OcsApiProvider default. */
  apiPrefix?: string
  /** Header title. */
  title?: string
  /** When set, hides the Division / CORP-team / DFW sub-team pickers and locks
   *  the report to division=CORP + this one CORP team (e.g. "TEAM1"),
   *  regardless of any ?div/?corp/?sub URL params. The backend router is also
   *  hard-locked — this is the UI half of the same lock. */
  lockedTeam?: string
  /** Header badge text (e.g. "CORP · T1"). */
  badge?: string
}

/**
 * Shared content for the OPs Customer Score report and its 4 per-team CORP
 * siblings (Bruno 2026-06-26). `apiPrefix` flows through `OcsApiProvider` so
 * every hook automatically points at the right backend router. `lockedTeam`
 * swaps the Division / team pickers for a static badge and pins the scope to
 * CORP + that team. Suspense lives here because the body reads useSearchParams.
 */
export function CustomerScoreContent({
  apiPrefix = "custom/ops-customer-score",
  title = "OPs Customer Score",
  lockedTeam,
  badge,
}: Props) {
  return (
    <OcsApiProvider prefix={apiPrefix}>
      <Suspense
        fallback={
      ...
      >
        <OcsBody title={title} lockedTeam={lockedTeam} badge={badge} />
      </Suspense>
    </OcsApiProvider>
  )
}
...
function OcsBody({
  ...
  // Defense in depth: the locked variant always sends division=CORP +
  // teams=[lockedTeam]; the backend router also hard-locks these.
  ...
  const lockedBadge = badge ?? `CORP · ${lockedTeam}`

  return (
    <div className="min-h-screen bg-[#F9FAFB]">
      {/* Desktop-only banner */}
      <div className="hidden bg-[#FEF3C7] px-4 py-2 text-xs text-[#92400E] [@media(max-width:1279px)]:block">
        {title} is desktop-only. Open this page on a screen wider than 1280px.
      </div>
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-4">
        ...
        <div className="sticky top-0 z-10 mb-4 rounded-md border border-[#E5E7EB] bg-white p-3 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            {/* Locked-team badge (replaces Division + team pickers) */}
            {locked ? (
              <div>
                ...
                <div className="mt-1 flex items-center gap-2">
                  <span className="rounded-md border border-[#111827] bg-[#111827] px-3 py-1 text-xs text-white">
                    {lockedBadge}
                  </span>
