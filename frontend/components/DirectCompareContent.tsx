...
interface Props {
  /** Backend router prefix, e.g. "custom/ops-direct-compare-t1". Defaults to
   *  the cross-team router via the DcApiProvider default. */
  apiPrefix?: string
  /** Header title. */
  title?: string
  /** When set, both panels are locked to division=CORP + this single team and
   *  the Division/Team-pill UI is hidden (replaced with a static badge). The
   *  backend router is also hard-locked — this is the UI half of the same lock. */
  lockedTeam?: string
...
/**
 * Shared content for OPs Direct Compare and its 4 per-team CORP siblings
 * (Bruno 2026-06-26). The `apiPrefix` flows through `DcApiProvider` so every
 * hook automatically points at the right backend router. `lockedTeam` pins both
 * panels to one CORP team and swaps the Division/Team pickers for a static badge
 * (defense in depth — the backend hard-locks too). The two panels keep their
 * independent date-range pickers.
 */
export function DirectCompareContent({
  apiPrefix = "custom/ops-direct-compare",
...
function Body({
  ...
  // When a team is locked, both panels always send division=CORP + [lockedTeam]
  // (defense in depth; the backend hard-locks regardless of what we send).
  const lockFilters = useCallback(
    (f: DCPanelFilters): DCPanelFilters =>
      lockedTeam
        ? { ...f, division: "CORP", teams: [lockedTeam], subTeams: undefined }
        : f,
    [lockedTeam],
  )

  const f1 = useMemo(() => lockFilters(panelToFilters(p1)), [p1, lockFilters])
  const f2 = useMemo(() => lockFilters(panelToFilters(p2)), [p2, lockFilters])
