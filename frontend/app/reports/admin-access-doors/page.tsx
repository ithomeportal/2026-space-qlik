...
import {
  ...
  fmtExpectedTime,
  checkColorClass,
  type AdminAccessFilters,
} from "@/lib/admin-access-api"
import { AdminErrorBanner } from "./ErrorBanner"
import { AdminTrendChart } from "./TrendChart"
...
export default function AdminAccessDoorsPage() {
  return (
    <ReportGuard reportKey="admin-access-doors">
      <AdminAccessDoorsContent />
    </ReportGuard>
  )
}

function AdminAccessDoorsContent() {
  ...
  const { data: filterRes, isLoading: loadingFilters } = useAdminFilters()
  const options = filterRes?.data

  const filters: AdminAccessFilters = useMemo(
    () => ({
      startDate,
      endDate,
      name: name || undefined,
    ...
    [startDate, endDate, name, jobTitle],
  )

  const kpisQ = useAdminKpis(filters)
  const rowsQ = useAdminRows({ ...filters, sort, page, limit: PAGE_LIMIT })
  const trendQ = useAdminTrend30d({ name: filters.name, jobTitle: filters.jobTitle })
  const jobTitleQ = useAdminByJobTitle({
    ...
    <div className="flex min-h-[calc(100vh-64px)] flex-col bg-[#F9FAFB]">
      ...
      <div className="flex items-center gap-3 border-b border-[#E5E7EB] bg-white px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold text-[#1B3A5C]">Admin - Access Log Doors</h1>
      ...
      <div className="sticky top-0 z-10 border-b border-[#E5E7EB] bg-white shadow-sm">
        <div className="mx-auto flex w-full max-w-[1920px] flex-wrap items-center gap-4 px-6 py-3">
          ...
          {(name || jobTitle || startDate !== today || endDate !== today) && (
            <button
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-[#E5E7EB] bg-white p-3">
            ...
            {trendQ.isLoading ? (
              ...
              <AdminTrendChart points={trendQ.data?.data ?? []} />
            )}
          </div>
          <div>
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#374151]">
            ...
            {jobTitleQ.isLoading ? (
              ...
              <JobTitleBarChart rows={jobTitleQ.data?.data ?? []} />
            )}
          </div>
        </section>
