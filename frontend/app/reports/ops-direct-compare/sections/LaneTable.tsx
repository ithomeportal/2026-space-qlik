...
interface Props {
  title: string
  panel: "p1" | "p2"
  panelFilters: DCPanelFilters
  diffAgainst?: DCPanelFilters
}

export function LaneTable({ title, panel, panelFilters, diffAgainst }: Props) {
  const [sort, setSort] = useState<string>(
    diffAgainst ? "p2_profit_desc" : "profit_desc",
  )
  const single = useDCByLane(panel, panelFilters, { sort, limit: 200 })
  const diff = useDCByLaneDiff(diffAgainst ?? panelFilters, panelFilters, {
    sort,
    limit: 200,
  })
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map((r, idx) => (
                ...
                >
                  <td className="px-2 py-1.5 text-[#111827]" title={r.lane}>
                    {r.lane}
                  </td>
