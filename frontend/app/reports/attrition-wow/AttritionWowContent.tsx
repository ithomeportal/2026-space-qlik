...
export interface AttritionWowContentProps {
  /** Backend router prefix, e.g. "custom/attrition-wow-t1". Defaults to the
   *  cross-team parent. Also seeds every queryKey (see attrition-wow-api). */
  apiPrefix?: string
  title?: string
  /** When set, hides the Teams controls and locks the report to one CORP team
   *  (e.g. "TEAM1"). The backend router is hard-locked too — this is the UI
   *  half of the same lock, and it also neutralises ?teams= / ?view=ruan so a
   *  stale URL param can never scope (or widen) the data invisibly. */
  lockedTeam?: string
  badge?: string
}

function AttritionBody({
  title = "Attrition WoW",
  lockedTeam,
  badge = "week-over-week",
}: AttritionWowContentProps) {
  ...
  // ⚠ RUAN forces team_id='TEAM-DFW' server-side, so it would punch straight
  // through a CORP team lock — it is unavailable on the locked copies.
  const view = !locked && searchParams.get("view") === "ruan" ? "ruan" : undefined
  const isRuan = view === "ruan"
  const entityLabel = isRuan ? "Client" : "Customer"
  const teamsParam = searchParams.get("teams")
  const teams = useMemo<string[]>(() => {
    if (lockedTeam) return [lockedTeam]
    if (teamsParam === null) return [...ALL_TEAMS]
    if (teamsParam === "") return []
    return teamsParam
      .split(",")
      .filter((t) => (ALL_TEAMS as readonly string[]).includes(t))
  }, [teamsParam, lockedTeam])
      ...
      <div className="sticky top-0 z-10 border-b border-[#E5E7EB] bg-white shadow-sm">
        <div className="mx-auto flex w-full max-w-[1920px] flex-wrap items-center gap-4 px-6 py-3">
          <div className="flex items-center gap-2">
            ...
            {lockedTeam ? (
              // Locked copies show a static badge — the pills would imply a
              // choice the server will not honour.
              ...
              <>
                <div className="flex rounded-lg border border-[#E5E7EB] bg-[#F9FAFB] text-xs">
                  ...
                  />
                  <TeamModeBtn label="RUAN" active={isRuan} onClick={setRuan} />
                </div>
...
/** Shared report body. `<Suspense>` is mandatory — the body calls
 *  `useSearchParams()`, which forces dynamic rendering. */
export function AttritionWowContent(props: AttritionWowContentProps) {
  return (
    <AttritionApiProvider prefix={props.apiPrefix}>
      <Suspense
        fallback={
