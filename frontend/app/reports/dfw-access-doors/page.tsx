...
import {
  ...
  fmtExpectedTime,
  checkColorClass,
  type DfwAccessFilters,
} from "@/lib/dfw-access-api"
...
function DfwAccessDoorsContent() {
  ...
  const { data: filterRes, isLoading: loadingFilters } = useDfwFilters()
  const options = filterRes?.data

  const filters: DfwAccessFilters = useMemo(
    () => ({
      startDate,
      endDate,
      name: name || undefined,
  ...
  const kpisQ = useDfwKpis(filters)
  const rowsQ = useDfwRows({ ...filters, sort, page, limit: PAGE_LIMIT })
  const trendQ = useDfwTrend30d({ name: filters.name, jobTitle: filters.jobTitle })
  const jobTitleQ = useDfwByJobTitle({
    ...
    <div className="flex min-h-[calc(100vh-64px)] flex-col bg-[#F9FAFB]">
      ...
      <div className="flex items-center gap-3 border-b border-[#E5E7EB] bg-white px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold text-[#1B3A5C]">DFW - Access Log Doors</h1>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-[#E5E7EB] bg-white p-3">
            ...
            {trendQ.isLoading ? (
              ...
              <DfwTrendChart points={trendQ.data?.data ?? []} />
            )}
          </div>
          <div>
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#374151]">
