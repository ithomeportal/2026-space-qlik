...
import {
  ...
  fmtPct,
  fmtUsd,
  useXrayDfwWeeklyPerformance,
  type XrayDfwFilters,
  type XrayDfwWeekPerf,
} from "@/lib/xray-dfw-api"

interface Props {
  filters: Pick<XrayDfwFilters, "subTeams" | "customers" | "lanes" | "view">
  onClose: () => void
}

type Kind = "count" | "usd" | "pct"

// Bruno 2026-06-10: the production rows from the Ops Portal Overview Team
// Weekly modal he screenshotted, ending at Lates DEL.
const ROWS: {
  label: string
  field: keyof XrayDfwWeekPerf
  kind: Kind
  signed?: boolean
  highlight?: boolean
  band?: boolean
...
export function WeeklyPerformanceModal({ filters, onClose }: Props) {
  const cf = {
  ...
  const { data, isLoading, error } = useXrayDfwWeeklyPerformance(cf, true)
  const weeks: XrayDfwWeekPerf[] = data?.data?.weeks ?? []
      ...
      >
        <div className="flex items-center justify-between border-b border-[#E5E7EB] bg-[#F0F9FF] px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#6B7280]">· last 8 weeks (Mon–Sun)</span>
          </div>
          <button
            onClick={onClose}
                  ...
                  >
                    <td
                      className={`px-2 py-1.5 ${
                        row.highlight ? "font-semibold text-[#1B3A5C]" : "text-[#6B7280]"
                      }`}
                    >
                      {row.label}
                    </td>
