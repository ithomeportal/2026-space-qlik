...
import type { HrTrendPoint } from "@/lib/hr-access-api"
import { fmtDate } from "@/lib/hr-access-api"

interface Props {
  points: HrTrendPoint[]
}

// Dual-line SVG chart (On Time green / Out of Time red) over the previous
// 30 calendar days. Matches Bruno's Qlik "On time – out of time trend".
// Inline SVG (no external lib) — follows the eSavings MonthlyTotalsChart
// pattern so we don't pull in recharts just for one panel.
...
export function HrTrendChart({ points }: Props) {
  ...
  return (
    <div className="relative">
      <div className="mb-1 flex items-center gap-4 text-[10px] text-[#374151]">
        ...
        <span className="ml-auto text-[#9CA3AF]">Rolling 30 days · ignores date filter</span>
      </div>
