...
export default function HrAccessDoorsPage() {
  return (
    <ReportGuard reportKey="hr-access-doors">
...
function HrAccessDoorsContent() {
  ...
  const { data: filterRes, isLoading: loadingFilters } = useHrFilters()
  ...
  const filters: HrFilters = useMemo(
    ...
    [startDate, endDate, department, name, jobTitle],
  )

  const kpisQ = useHrKpis(filters)
  const rowsQ = useHrRows({ ...filters, sort, page, limit: PAGE_LIMIT })
  const trendQ = useHrTrend30d({ name: filters.name, jobTitle: filters.jobTitle })
  const deptQ = useHrByDepartment({
    ...
    <div className="flex min-h-[calc(100vh-64px)] flex-col bg-[#F9FAFB]">
      ...
      <div className="flex items-center gap-3 border-b border-[#E5E7EB] bg-white px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold text-[#1B3A5C]">HR - Access Log Doors</h1>
          <span className="rounded-full bg-[#CCFBF1] px-2 py-0.5 text-xs text-[#115E59]">HR</span>
        </div>
        <div className="ml-auto text-xs text-[#6B7280]">
          {startDate === endDate ? startDate : `${startDate} → ${endDate}`}
          {department ? ` · ${department}` : " · All depts"}
          {name ? ` · ${name}` : ""}
        </div>
      </div>
      ...
      <div className="sticky top-0 z-10 border-b border-[#E5E7EB] bg-white shadow-sm">
        <div className="mx-auto flex w-full max-w-[1920px] flex-wrap items-center gap-4 px-6 py-3">
          ...
          {(department || name || jobTitle || startDate !== today || endDate !== today) && (
            <button
      ...
      <div className="mx-auto w-full max-w-[1920px] flex-1 space-y-4 px-6 py-5">
        <HrErrorBanner
          errors={[kpisQ.error, rowsQ.error, trendQ.error, deptQ.error]}
        />
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-[#E5E7EB] bg-white p-3">
            ...
            {trendQ.isLoading ? (
              ...
              <HrTrendChart points={trendQ.data?.data ?? []} />
            )}
          </div>
          <div>
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#374151]">
              On Time – Out of Time by Department
            </h2>
            {deptQ.isLoading ? (
              <div className="rounded-lg border border-[#E5E7EB] bg-white p-3">
                <SkelBlock height={200} />
              ...
              <HrDeptBarChart rows={deptQ.data?.data ?? []} />
            )}
          </div>
        </section>
