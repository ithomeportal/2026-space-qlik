...
function adminQs(f: Partial<AdminAccessFilters>, extra?: Record<string, string>) {
...
export interface AdminAccessFilterOptions {
  job_titles: string[]
  names: string[]
  today: string
}
...
export interface AdminAccessTrendPoint {
  event_date: string
  on_time: number
  out_of_time: number
}

export interface AdminAccessJobTitleBar {
  job_title: string
  on_time: number
  out_of_time: number
  unscored: number
...
export function useAdminFilters() {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<AdminAccessFilterOptions>("custom/admin-access-doors/filters"),
    staleTime: 30 * 60 * 1000,
  })
}

export function useAdminKpis(f: AdminAccessFilters, enabled = true) {
  return useQuery({
    ...ADMIN_RETRY,
    enabled,
    queryKey: ["dfw-access", "kpis", f],
    queryFn: () =>
      apiFetch<AdminAccessKpis>(`custom/admin-access-doors/kpis${adminQs(f)}`),
  })
}

export function useAdminRows(
  f: AdminAccessFilters & { sort?: string; page?: number; limit?: number },
  enabled = true,
) {
  return useQuery({
    ...ADMIN_RETRY,
    ...
    queryFn: () =>
      apiFetch<AdminAccessRow[]>(
        `custom/admin-access-doors/rows${adminQs(f, {
          ...(f.sort ? { sort: f.sort } : {}),
...
export function useAdminTrend30d(
  f: Pick<AdminAccessFilters, "name" | "jobTitle">,
  enabled = true,
) {
  return useQuery({
    ...ADMIN_RETRY,
    ...
    queryFn: () =>
      apiFetch<AdminAccessTrendPoint[]>(
        `custom/admin-access-doors/trend-30d${adminQs({
          startDate: "",
          endDate: "",
          name: f.name,
          jobTitle: f.jobTitle,
...
export function useAdminByJobTitle(
  f: Omit<AdminAccessFilters, "jobTitle">,
  enabled = true,
) {
  return useQuery({
    ...ADMIN_RETRY,
    ...
    queryFn: () =>
      apiFetch<AdminAccessJobTitleBar[]>(
        `custom/admin-access-doors/by-job-title${adminQs(f)}`,
      ),
  })
}
